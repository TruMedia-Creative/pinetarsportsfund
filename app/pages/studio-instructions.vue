...
useSeoMeta({
  ...
  description:
    'Step-by-step guide for creating, editing, and publishing investment opportunities and operational project pages with Nuxt Studio.',
});
...
const editorFaq = [
  {
    ...
    answer:
      'Open the image field in Studio and use the media picker. Replace only the value for the image field you want to change.',
    fields: [
      'cover.heroImageUrl',
  ...
  {
    question:
      'How do I edit bullets like Proven Phase I, Anchor Tenant, Scarcity Value, and Experienced Operator?',
    ...
    appearsIn:
      'investmentThesis.bullets renders in the Investment Thesis section; opportunity.bullets renders in the Phases/Opportunity section.',
  },
  {
    ...
    appearsIn:
      'Timeline phase chips render in Returns, while body copy renders in the Opportunity and Project Overview sections.',
  },
  {
    question: 'Where do I edit text for Dugout Howe Complex Phase II consists of ... ?',
    answer:
      'That text is not in Phase I budget fields. It is stored in the project overview body content.',
    fields: ['projectOverview.body'],
...
<template>
  <div>
    ...
    >
      <template #title> Studio Guide: Investment & Project Pages </template>
      <template #description>
        Use the tabs below to find the right workflow — creating or updating fundraising investment
        decks, or filling in data for an operational project facility.
      </template>
    </UPageHero>
        ...
        <template #investments>
          <div class="pt-10 space-y-16">
            ...
            >
              <UCard :ui="{ body: 'p-6' }">
                <ul class="space-y-2 text-sm">
                  <li v-for="field in requiredFields" :key="field" class="flex items-start gap-2">
                    <UIcon name="i-lucide-check-circle-2" class="size-4 mt-0.5 text-primary" />
                    <span>{{ field }}</span>
                  </li>
                </ul>
              </UCard>
            ...
            >
              <UCard :ui="{ body: 'p-6' }">
                <ul class="space-y-2 text-sm">
                  <li v-for="note in editNotes" :key="note" class="flex items-start gap-2">
                    <UIcon name="i-lucide-file-pen-line" class="size-4 mt-0.5 text-primary" />
                    <span>{{ note }}</span>
                  </li>
                </ul>
              </UCard>
            ...
            >
              <div class="space-y-4">
                ...
                >
                  ...
                  <div>
                    ...
                    <ul class="space-y-1 text-sm">
                      <li v-for="field in item.fields" :key="field" class="flex items-start gap-2">
                        <UIcon
                ...
                <UCard :ui="{ body: 'p-6' }">
                  <p class="text-sm font-semibold mb-2">Media Field Tips</p>
                  <ul class="space-y-2 text-sm">
                    <li v-for="tip in mediaFieldTips" :key="tip" class="flex items-start gap-2">
                      <UIcon name="i-lucide-image" class="size-4 mt-0.5 text-primary" />
                      <span>{{ tip }}</span>
                    </li>
                  </ul>
                </UCard>
            ...
            >
              <UCard :ui="{ body: 'p-0 overflow-hidden' }">
                <pre
                  class="overflow-x-auto p-5 text-xs sm:text-sm bg-elevated text-toned"
                ><code>{{ starterTemplate }}</code></pre>
              </UCard>
            </UPageSection>
            ...
            >
              <UCard :ui="{ body: 'p-6' }">
                <ul class="space-y-2 text-sm">
                  <li v-for="item in publishChecklist" :key="item" class="flex items-start gap-2">
                    <UIcon name="i-lucide-circle-check" class="size-4 mt-0.5 text-success" />
                    <span>{{ item }}</span>
                  </li>
                </ul>
              </UCard>
        ...
        <template #projects>
          <div class="pt-10 space-y-16">
              ...
              <UCard :ui="{ body: 'p-6' }">
                <ul class="space-y-2 text-sm">
                  <li
                    ...
                    class="flex items-start gap-2"
                  >
                    <UIcon name="i-lucide-bar-chart-2" class="size-4 mt-0.5 text-primary" />
                    <span class="font-mono text-xs sm:text-sm">{{ field }}</span>
                  </li>
                </ul>
              </UCard>
            ...
            >
              <UCard :ui="{ body: 'p-6' }">
                <ul class="space-y-2 text-sm">
                  <li v-for="tip in projectsMediaTips" :key="tip" class="flex items-start gap-2">
                    <UIcon name="i-lucide-image" class="size-4 mt-0.5 text-primary" />
                    <span>{{ tip }}</span>
                  </li>
                </ul>
            ...
            >
              <UCard :ui="{ body: 'p-0 overflow-hidden' }">
                <pre
                  class="overflow-x-auto p-5 text-xs sm:text-sm bg-elevated text-toned"
                ><code>{{ projectsStarterTemplate }}</code></pre>
              </UCard>
            </UPageSection>
