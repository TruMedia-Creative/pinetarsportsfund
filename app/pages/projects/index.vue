...
const visibleProjects = computed(() => (projects.value ?? []).filter((d) => d.published));
...
useSeoMeta({
  ...
  description:
    'Explore our portfolio of operational youth sports complexes across Texas — all fully built and generating revenue.',
});
</script>
...
<template>
  <div>
    ...
    >
      <template #title> Operational Projects </template>
      <template #description>
        Our portfolio of fully operational youth sports complexes across Texas. Each facility is
        generating active revenue through tournaments, leagues, and community programming.
      </template>
    </UPageHero>

    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
      <div v-if="visibleProjects.length" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <NuxtLink
          ...
          >
            <div class="flex items-start justify-between gap-4">
              ...
              <div class="flex flex-col items-end gap-1.5 shrink-0">
                <UBadge color="success" variant="subtle" size="sm"> Operational </UBadge>
                <UBadge
            ...
            <p v-if="project.subtitle" class="text-sm text-dimmed leading-relaxed">
              {{ project.subtitle }}
            </p>
            ...
            <div class="flex items-center justify-between pt-2 border-t border-default">
              <p class="text-xs font-mono text-dimmed flex items-center gap-1">
                <UIcon v-if="project.location?.city" name="i-lucide-map-pin" class="size-3" />
                {{
      ...
      <div v-else class="text-center py-24">
        <UIcon name="i-lucide-hard-hat" class="size-12 text-dimmed mx-auto mb-4" />
        <h3 class="text-lg font-semibold mb-2">Projects coming soon</h3>
        <p class="text-dimmed text-sm max-w-sm mx-auto">
          Our operational project portfolio will be listed here. Check back soon or contact us to
          learn more.
        </p>
        <UButton
          label="Contact Us"
