  ...
  <div class="flex min-h-screen items-center justify-center bg-zinc-950 px-4">
    <div class="w-full max-w-sm rounded-xl border border-zinc-800 bg-zinc-900 p-10 shadow-2xl">
      <div class="mb-6 text-center">
        <h1 class="text-2xl font-semibold text-white">Content Studio</h1>
        <p class="mt-1 text-sm text-zinc-400">
          <template v-if="state === 'loading'"> Signing you out. </template>
          <template v-else-if="state === 'success'"> You have been signed out. </template>
          <template v-else> We could not complete logout. </template>
        </p>
      </div>
      ...
      <div v-else class="space-y-4">
        <div role="alert" class="rounded-lg bg-red-500/10 px-4 py-3 text-sm text-red-400">
          {{ error }}
        </div>
      ...
      <p class="mt-6 text-center text-xs text-zinc-500">Pine Tar Sports Fund - Content Studio</p>
    </div>
  </div>
</template>
