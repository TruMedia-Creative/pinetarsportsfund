    ...
    async function initializeImmersiveScroll() {
      ...
      const context = gsap.context(() => {
        ...
        media.add('(min-width: 1024px)', () => {
          const atmosphere = stageElement.querySelector<HTMLElement>(
            '.homepage-immersive-atmosphere'
          );
          const orbits = Array.from(
            stageElement.querySelectorAll<HTMLElement>('[data-home-orbit]')
          );
          ...
          scenes.forEach((scene, index) => {
            ...
            if (!isLastScene) {
              timeline.to(content, { y: -40, autoAlpha: 0.6, scale: 1.015, duration: 0.42 }, 0.58);
            }
