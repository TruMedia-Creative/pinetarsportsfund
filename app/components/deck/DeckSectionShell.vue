...
const showSidebar = computed(
  () => hasImage.value && (layout.value === 'left' || layout.value === 'right')
);
const captionStyle = computed(() => props.sectionImage?.captionStyle || 'below');
const showOverlayCaption = computed(
  () =>
    Boolean(props.sectionImage?.caption) &&
    (captionStyle.value === 'overlay' || captionStyle.value === 'both')
);
const showBelowCaption = computed(
  () =>
    Boolean(props.sectionImage?.caption) &&
    (captionStyle.value === 'below' || captionStyle.value === 'both')
);
...
<template>
  <div class="space-y-8">
    <div v-if="showBanner" class="rounded-2xl overflow-hidden border border-default">
      ...
      <p v-if="showBelowCaption" class="px-4 py-3 text-sm text-dimmed border-t border-default">
        {{ sectionImage?.caption }}
      </p>
    </div>
    ...
    <div v-if="showSidebar && layout === 'left'" class="grid lg:grid-cols-2 gap-12 items-start">
