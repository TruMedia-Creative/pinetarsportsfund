...
<template>
  <section v-if="resolvedEmbedUrl || fullAddress" class="py-20 border-b border-default bg-muted/20">
    <div class="max-w-6xl mx-auto px-6">
      <div class="mb-8">
        <h2 class="text-2xl font-bold mb-1">Location</h2>
        <p v-if="fullAddress" class="text-dimmed flex items-center gap-2">
      ...
      <div v-else class="rounded-2xl border border-default p-8 text-center text-dimmed">
        <UIcon name="i-lucide-map" class="size-10 mx-auto mb-3 opacity-40" />
        <p class="text-sm">Map coming soon</p>
      </div>

      <p v-if="caption" class="text-xs text-dimmed mt-3 text-center">
        {{ caption }}
      </p>
    </div>
  </section>
